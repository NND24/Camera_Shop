...
if (isset($_POST['searchInput'])) {
    ...
    $sql = "SELECT * FROM tbl_user WHERE name LIKE '%{$input}%' LIMIT " . $item_per_page . " OFFSET " . $offset . "";
    ...
    $totalRecords = mysqli_query($mysqli, "SELECT * FROM tbl_user WHERE name LIKE '%{$input}%'");
...
?>
            <div class="products-row">
                ...
                <div class="product-cell col-1 btn">
                    <button title="Xóa" class="remove-user" value="<?php echo $row['id_user'] ?>"><i class="fa-solid fa-trash"></i></button>
                </div>
                <?php //} 
                ?>
            </div>
