<div class="row no-wrap products-category">
    ...
    ?>
        ...
        $sql_pro = "SELECT * FROM tbl_sanpham WHERE tbl_sanpham.soluong>0 AND tbl_sanpham.id_danhmuc='$_GET[id]' AND tbl_sanpham.giadagiam >= 500000 AND tbl_sanpham.giadagiam <= 2000000 ORDER BY id_sanpham ASC  LIMIT " . $item_per_page . " OFFSET " . $offset . "";
        $query_pro = mysqli_query($mysqli, $sql_pro);

        $totalRecords = mysqli_query($mysqli, "SELECT * FROM tbl_sanpham WHERE tbl_sanpham.soluong>0 AND tbl_sanpham.id_danhmuc='$_GET[id]' AND tbl_sanpham.giadagiam >= 500000 AND tbl_sanpham.giadagiam <= 2000000");
        $totalRecords = mysqli_num_rows($totalRecords);
        $totalPages = ceil($totalRecords / $item_per_page);
        if (mysqli_num_rows($query_pro) > 0) {
            while ($row_pro = mysqli_fetch_array($query_pro)) {
